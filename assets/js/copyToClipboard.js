...
function getNotificationText() {
  ...
  const language =
    ...
    (path.includes("/r/") ? "ru" : path.includes("/read/") ? "en" : "en");
...
function copyToClipboard(text = "") {
  ...
  if (clickedLang !== "pi") {
    const translationElement = clickedElement.closest(
      '[lang]:not([lang="pi"])'
    );
  ...
  if (clickedLang !== "pi") {
    const otherTranslations = Array.from(
      parentSpan.querySelectorAll(
        '[lang]:not([lang="pi"]):not([lang="' + clickedLang + '"])'
      )
    )
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const handleLineLinkCopy = (event) => {
    ...
    // Предотвращаем стандартное поведение (меню, выделение текста, зум)
  ...
  // 1. Обработчик для правого клика мыши (desktop)
  ...
  // 2. НОВЫЙ ОБРАБОТЧИК ДЛЯ ДВОЙНОГО КЛИКА (desktop и mobile)
  ...
  // 3. Обработчики для долгого нажатия (mobile)
  ...
  document.addEventListener("touchmove", (event) => {
    ...
    if (
      Math.abs(moveX - startX) > pixelThreshold ||
      Math.abs(moveY - startY) > pixelThreshold
    ) {
      clearLongPressTimer();
    }
  });
